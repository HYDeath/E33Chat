...
object ListenerChat {
    ...
    private fun isDisabledWorld(player: Player): Boolean {
        if (worldPatterns.isEmpty()) return false
    ...
    fun onPaperChat(e: AsyncChatEvent) {
        ...
        // TrChat owns delivery on Paper/Folia. Cancelling before dispatch prevents
        // vanilla delivery from racing the entity-scheduled channel output.
        e.isCancelled = true

        val player = e.player
        val message = e.message().toNative()
        val prefixed = cachePrefix.remove(player.uniqueId)
        player.scheduler.run(bukkitPlugin, {
            if (isDisabledWorld(player)) return@run
            val channel = prefixed?.let { Channel.channels[it] } ?: player.session.getChannel()
            channel?.execute(player, message, toConsole = true)
        }, null)
    }
