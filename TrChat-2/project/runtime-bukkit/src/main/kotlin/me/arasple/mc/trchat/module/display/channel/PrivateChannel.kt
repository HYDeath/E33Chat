...
class PrivateChannel(
    ...
    override fun execute(player: Player, message: ComponentText, toConsole: Boolean): ChannelExecuteResult {
        Function.clearMentioned()
        E33Bridge.rememberMessage(player.uniqueId, null)
        E33Bridge.rememberOriginalLinks(player.uniqueId, message.toPlainText())
        ...
        if (Bukkit.getPlayerExact(to) == null && !BukkitProxyManager.canDeliverRemotePrivate()) {
            player.sendMessage("§c跨服私信暂不可用：Redis 未连接，消息没有发出。")
            return ChannelExecuteResult(failedReason = ChannelExecuteResult.FailReason.NO_RECEIVER)
        }
        session.lastChannel = this
        session.lastPrivateMessage = plain
        // Native CustomNameplates' TrChat provider only bubbles forwarded events.
        val event = TrChatEvent(this, session, message, forward = false)
        if (!event.call()) {
            return ChannelExecuteResult(failedReason = ChannelExecuteResult.FailReason.EVENT)
        }
        var msg = events.process(player, event.component)
            ?: return ChannelExecuteResult(failedReason = ChannelExecuteResult.FailReason.EVENT)
        plain = TrChat.api().getFilterManager().filter(msg.toPlainText()).filtered
        if (plain != msg.toPlainText()) msg = Components.text(plain)
        if (!player.data.isShadowMuted) E33Bridge.rememberMessage(player.uniqueId, plain)
        ...
        E33Bridge.rememberBody(player.uniqueId, msgComponent.toAdventureObject())
        val mentioned = Function.takeMentioned()
        ...
        var senderEcho: ComponentText? = null
        if (senderEvent.call()) {
            send = senderEvent.component
            senderEcho = send
            E33Bridge.rememberRendered(player.uniqueId, send.toAdventureObject())
            if (!settings.proxy || BukkitProxyManager.processor == null || player.data.isShadowMuted)
                player.sendComponent(player, send)
        }

        E33Bridge.recordReply(to, player.name)
        ...
        if (Components.useAdventure) {
            receive = taboolib.module.chat.impl.AdventureComponent(
                E33Bridge.expandCraftEmoji(player, receive.toAdventureObject()))
        }
        E33Bridge.rememberRendered(player.uniqueId, receive.toAdventureObject())
        ...
        if (settings.proxy && BukkitProxyManager.processor != null) {
            BukkitProxyManager.sendPrivateRaw(
                ...
                receive,
                msgComponent,
                bridgeChat = E33Bridge.stagedBase64(player.uniqueId),
                senderEcho = senderEcho
            )
            ...
            getProxyPlayer(to)?.let {
                ...
                if (!E33Bridge.isV1Client(to)) it.sendLang("Private-Message-Receive", player.name)
                if (to in mentioned) {
