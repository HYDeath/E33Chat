...
object TrChatBukkit : Plugin() {
    ...
    internal fun detectPaperEnv() {
        val modern26 = runCatching { Bukkit.getBukkitVersion().startsWith("26.") }.getOrDefault(false)
        if ((hasClass("com.destroystokyo.paper.PaperConfig")
            || hasClass("io.papermc.paper.configuration.Configuration"))
            && (versionId >= 11604 || modern26)) {
            isPaperEnv = true
        }
        if (Folia.isFolia || (isPaperEnv && (modern26 || MinecraftVersion.isHigherOrEqual(MinecraftVersion.V1_20)))) {
    ...
    internal fun onConst() {
        detectPaperEnv()
        // ConfigNodeLoader also runs at INIT. Migrate existing files first so
        // its missing-node warnings are not emitted for newly added settings.
        runCatching { YamlUpdater.update("settings.yml", updateExists = false) }
    }
