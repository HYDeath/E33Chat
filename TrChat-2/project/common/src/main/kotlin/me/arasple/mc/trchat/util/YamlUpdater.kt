...
object YamlUpdater {
    ...
    private fun read(cache: ConfigurationSection, to: ConfigurationSection, skipNodes: Array<String>, updated: MutableList<String>, updateExists: Boolean) {
        ...
        for (key in cache.getKeys(false)) {
            ...
            // Existing sections still need to be visited when only filling missing
            // nodes. Otherwise newly introduced nested settings never migrate.
            if (!updateExists) {
                val sourceSection = cache[key]
                val targetSection = to[key]
                if (sourceSection is ConfigurationSection && targetSection is ConfigurationSection) {
                    read(sourceSection, targetSection, skipNodes, updated, false)
                }
                continue
            }
