...
## v2.4.15+compat.28 (Minecraft 26.x)

- 恢复升级 2.4.15 时遗漏的 B站与 uwqq.com 链接卡片：同步补回高度、宽度、封面、标题、整卡点击和缩放后的点击区域。
- 链接消息不再套用只能容纳文字的 CustomNameplates 气泡；系统宣传消息和带图片的链接消息也可显示卡片。
- B站与官网链接点击直接打开，其他 HTTP(S) 链接显示确认框；保留完整跳转参数，BV/AV 预览请求仍使用简化网址。
- 元数据或封面加载期间保留可点击的站点卡片，网络失败也不会让卡片消失。
- 26.1.2、26.2、26.3 构建通过；链接回归检查覆盖发送回显、接收方短标签、继承的点击样式、完整参数和可信域名判定。尚未完成双客户端联机画面验证。

## v2.4.15+compat.27 (Minecraft 26.x)

- 修复服务器聊天中 `run_command` 点击失效（例如接受传送、抢红包）：改用 Minecraft 的游戏内点击处理器，原先调用的通用处理器不支持执行命令。
- 修正 26.x 构建时 `Screen.handleClickEvent` 的 API 映射，保留原版命令、对话框和自定义点击动作的处理。
- 此次仅更新 26.1.2、26.2、26.3 的 Fabric 模组；1.21.11 已使用正确的处理器，TrChat 无需更新。
- 26.1.2 开发客户端实测：渲染后的聊天文字点击执行 `/help` 并收到服务端响应；填入命令、复制文本及拖选时不执行命令均通过。具体红包、传送插件仍需在正式服务器确认。

## v2.3.15+compat.2

- 修复 26.1.2 启动崩溃：Mixin 不允许 `ChatListenerMixin` 注入非私有静态方法；将兜底解析器改为私有方法，并由聊天栏通过反射安全调用。相同修复同时应用于 26.2 和 26.3。
- 此版本仍须进行客户端启动和聊天功能实测；Gradle 构建通过不等同于运行时认证。
