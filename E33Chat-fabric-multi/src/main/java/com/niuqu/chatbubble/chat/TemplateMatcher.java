...
public class TemplateMatcher {
    ...
    public static Optional<TemplateResult> match(String text, List<CompiledTemplate> chatTpls,
        ...
        return match(text, chatTpls, whisperTpls, resolver, content -> false);
    }

    public static Optional<TemplateResult> match(String text, List<CompiledTemplate> chatTpls,
            List<CompiledTemplate> whisperTpls, NameResolver resolver,
            Predicate<String> serverVerifiedContent) {
        if (text == null || text.isEmpty()) return Optional.empty();
        for (CompiledTemplate t : whisperTpls) {
            TemplateResult r = tryMatch(text, t);
            if (r == null) continue;
            if (r.sender() != null && (resolver.isKnown(r.sender()) || serverVerifiedContent.test(r.content())))
                return Optional.of(withVerified(r, r.sender()));
            if (r.target() != null && (resolver.isKnown(r.target()) || serverVerifiedContent.test(r.content())))
        ...
        for (CompiledTemplate t : chatTpls) {
            TemplateResult r = tryMatch(text, t);
            if (r == null) continue;
            if (r.displayName() != null && (resolver.isKnown(r.displayName()) || serverVerifiedContent.test(r.content())))
