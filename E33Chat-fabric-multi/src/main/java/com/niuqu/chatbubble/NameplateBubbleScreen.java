...
public final class NameplateBubbleScreen extends Screen {
    ...
    @Override
    protected void init() {
        GuiCompat.addDrawableChild(this, GuiCompat.button(Text.literal("返回聊天"),
            button -> client.setScreen(parent), width / 2 - 104, height - 30, 100, 20));
        GuiCompat.addDrawableChild(this, GuiCompat.button(Text.literal("刷新列表"),
            button -> refresh(), width / 2 + 4, height - 30, 100, 20));
        refresh();
    }
    ...
    public void extractRenderState(DrawContext g, int mouseX, int mouseY, float tickDelta) {
        ...
        if (!received && loading) {
            ...
            for (int i = 0; i < catalogue.size(); i++) {
                ...
                Text preview = ChatMessageStore.componentFromJson(entry.previewJson());
                if (preview != null) {
