...
public record BridgeChatV2Payload(UUID messageId, UUID sender, String account, String displayName,
                                ...
                                String recipient, String quoteSender, String quoteContent,
                                 ...
                                 String renderedJson) implements CustomPayload {
    public static final Id<BridgeChatV2Payload> ID = new Id<>(Identifier.of("e33chat", "bridge_chat_v2"));
    public static final PacketCodec<PacketByteBuf, BridgeChatV2Payload> CODEC = PacketCodec.of(
        //#if MC >= 26000
        (buf, value) -> write(buf, value),
        //#else
        //$$ (value, buf) -> write(buf, value),
        //#endif
        BridgeChatV2Payload::read);

    private static void write(PacketByteBuf buf, BridgeChatV2Payload value) {
        buf.writeVarInt(2);
        ...
        buf.writeString(value.displayJson);
        buf.writeString(value.renderedJson);
    }

    private static BridgeChatV2Payload read(PacketByteBuf buf) {
        if (buf.readVarInt() != 2) throw new IllegalArgumentException("Unsupported bridge protocol");
        ...
        String displayJson = buf.readString(8192);
        String renderedJson = buf.readString(16384);
        if (buf.readableBytes() != 0) throw new IllegalArgumentException("Trailing bridge chat data");
        return new BridgeChatV2Payload(id, sender, account, display, origin, body, json,
            privateMessage, recipient, quoteSender, quoteContent, mentions, nameplate,
            displayJson, renderedJson);
    }

    @Override public Id<BridgeChatV2Payload> getId() { return ID; }
}
//#endif
