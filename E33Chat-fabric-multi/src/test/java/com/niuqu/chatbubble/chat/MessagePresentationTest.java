...
class MessagePresentationTest {
    @Test void publicMentionOfWhisperNamedPlayerIsNotPrivate() {
        String line = "[ 基佬俱乐部♂ 副宗主♕ ] jojo ❏ 你这个一幅画要多久啊 @whisper_1217";
        assertFalse(MessagePresentation.hasWhisperKeywordBeforeColon(line));
        var publicChat = MessagePresentation.parseDecoratedPlayerLine(
            line, List.of("jojo", "whisper_1217")).orElseThrow();
        assertEquals("jojo", publicChat.playerName());
        assertEquals("你这个一幅画要多久啊 @whisper_1217", publicChat.content());
        assertFalse(MessagePresentation.hasWhisperKeywordBeforeColon("jojo » @whisper_1217"));
        assertFalse(MessagePresentation.hasWhisperKeywordBeforeColon("jojo: @whisper_1217"));
        assertTrue(MessagePresentation.hasWhisperKeywordBeforeColon("You whisper to Steve: hi"));
        assertTrue(MessagePresentation.hasWhisperKeywordBeforeColon("[私聊] jojo ❏ hi"));
    }
    ...
    @Test void parsesResourcePackSquareSeparatorFromPlainClient() {
        var parsed = MessagePresentation.parseDecoratedPlayerLine(
            "[全勤能手] Bishi_0v0 ▫ 我想换个机子了", List.of("Bishi_0v0"));
        assertEquals("我想换个机子了", parsed.orElseThrow().content());
    }

    @Test void parsesSinglePrivateUseSeparatorButNotBroadcastSentence() {
        var parsed = MessagePresentation.parseDecoratedPlayerLine(
            "Steve \uE001 你好", List.of("Steve"));
        assertEquals("你好", parsed.orElseThrow().content());
    ...
    @Test void joinAndQuitNoticesStaySystemBeforeTemplateMatching() {
        var names = List.of("chenzihao_", "§6Steve", "小明");
        assertTrue(MessagePresentation.isPlayerPresenceNotice("chenzihao_加入了游戏", names));
        assertTrue(MessagePresentation.isPlayerPresenceNotice("chenzihao_ 加入了游戏", names));
        assertTrue(MessagePresentation.isPlayerPresenceNotice("§6Steve§r joined the game", names));
        assertTrue(MessagePresentation.isPlayerPresenceNotice("[VIP] 小明 离开了游戏", names));
        assertFalse(MessagePresentation.isPlayerPresenceNotice("chenzihao_: 加入了游戏", names));
        assertFalse(MessagePresentation.isPlayerPresenceNotice("chenzihao_ ❏ 加入了游戏", names));
        assertFalse(MessagePresentation.isPlayerPresenceNotice("公告：chenzihao_加入了游戏", names));
    }

    @Test void whitespaceOnlyGapDoesNotTurnNoticeIntoPlayerChat() {
        assertTrue(MessagePresentation.parseDecoratedPlayerLine(
            "chenzihao_ 加入了游戏", List.of("§6chenzihao_", "chenzihao_")).isEmpty());
        assertEquals("加入了游戏", MessagePresentation.parseDecoratedPlayerLine(
            "chenzihao_ ❏ 加入了游戏", List.of("chenzihao_")).orElseThrow().content());
    }
    ...
    @Test void unicodeArrowSeparatorNotSkipped() {
        // An unknown arrow does not prove this is player chat.
        assertTrue(MessagePresentation.parseDecoratedPlayerLine(
            "Steve ➤ hi", List.of("Steve")).isEmpty());
    }
