...
public class ChatMessageStore {
    ...
    private static boolean projectingBridgeHud;
    private static final Map<UUID, Boolean> bridgeSeen = new LinkedHashMap<>(2048, .75f, true) {
    ...
    /** Authoritative semantic delivery from the existing TrChat bridge. */
    public static void addBridgeMessage(com.niuqu.chatbubble.network.BridgeChatPayload payload) {
        addBridgeMessage(new com.niuqu.chatbubble.network.BridgeChatV2Payload(
            payload.messageId(), payload.sender(), payload.account(), payload.displayName(),
            payload.origin(), payload.body(), payload.bodyJson(), payload.privateMessage(),
            payload.recipient(), payload.quoteSender(), payload.quoteContent(), payload.mentions(),
            payload.nameplate(), "", ""));
    }

    public static void addBridgeMessage(com.niuqu.chatbubble.network.BridgeChatV2Payload payload) {
        if (bridgeSeen.putIfAbsent(payload.messageId(), Boolean.TRUE) != null) return;
        if (BlockList.isPlayerBlocked(payload.account(), Text.literal(payload.displayName()),
                ChatBubbleClientSetup.config().blockedPlayers())) return;

        Text content = HistoryStore.componentFromJson(payload.bodyJson());
        if (content == null || content.getString().isBlank()) content = Text.literal(payload.body());
        ...
        boolean own = local != null && (payload.sender().equals(local.getUuid())
            || payload.account().equalsIgnoreCase(local.getName().getString()));
        List<String> mentions = local != null && payload.mentions().contains(local.getUuid())
            ? List.of(local.getName().getString()) : List.of();
        EchoTracker.putPendingMeta(String.valueOf(content.getString().hashCode()), payload.sender(),
        ...
        if (hud == null) hud = Text.empty().append(decorated).append(Text.literal(" » ")).append(content);
        var mc = MinecraftClient.getInstance();
        if (mc != null && mc.inGameHud != null) {
            projectingBridgeHud = true;
            try { mc.inGameHud.getChatHud().addClientSystemMessage(hud); }
            finally { projectingBridgeHud = false; }
        }
    }
    ...
    public static Text ownDisplayName() {
        ...
        if (player != null && player.getScoreboardTeam() != null) {
            ...
            //#if MC >= 260200
            //$$ if (hasPfx || hasSfx || col.isPresent()) {
            //#else
            if (hasPfx || hasSfx || col != null) {
            //#endif
                MutableText name = Text.literal(player.getName().getString());
                //#if MC >= 260200
                //$$ if (col.isPresent()) name = name.withStyle(style -> style.withColor(col.get().rgb()));
                //#else
