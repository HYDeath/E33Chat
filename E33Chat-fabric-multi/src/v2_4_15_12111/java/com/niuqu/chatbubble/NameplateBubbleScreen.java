...
public final class NameplateBubbleScreen extends Screen {
    ...
    @Override
    protected void init() {
        addDrawableChild(ButtonWidget.builder(Text.literal("返回聊天"),
            button -> client.setScreen(parent)).dimensions(width / 2 - 104, height - 30, 100, 20).build());
        addDrawableChild(ButtonWidget.builder(Text.literal("刷新列表"),
            button -> refresh()).dimensions(width / 2 + 4, height - 30, 100, 20).build());
        refresh();
    }
    ...
    @Override
    public void render(DrawContext g, int mouseX, int mouseY, float tickDelta) {
